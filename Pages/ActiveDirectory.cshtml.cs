...
public class ActiveDirectoryModel : DashboardPageModel
{
    public ActiveDirectoryModel(ActiveRolesService arService, UserSettingsService userSettingsService, IOptionsMonitor<ActiveRolesConfig> arConfig)
    ...
    public override async Task<IActionResult> OnGetAsync([FromQuery] bool cached = false)
    {
        ...
        // The AD dashboard honours the global segment selection (by domain) but does not
        ...
        // cached=true filter re-renders - so navigating here never re-runs the slow AD query.
